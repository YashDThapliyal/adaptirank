...
def expanded_findings_doc(
    ...
    ce_a_delta = mapped["hybrid_to_cross_encoder"]["ndcg_10"] - mapped["weighted_hybrid"]["ndcg_10"]
    ce_b_delta = (
        mapped["hybrid_to_lambdamart_to_cross_encoder"]["ndcg_10"] - mapped["lambdamart"]["ndcg_10"]
    )
    lm_delta = mapped["lambdamart"]["ndcg_10"] - mapped["weighted_hybrid"]["ndcg_10"]
    pw_delta = mapped["pointwise"]["ndcg_10"] - mapped["lambdamart"]["ndcg_10"]
    sec = []
    sec.append(
        (
    ...
    sec.append(
        (
            ...
            f"Observed result: dataset fingerprint `{source['dataset_fingerprint']}`. CE root `{paths['ce']}`. Analysis root `{paths['analysis']}`. Pair union SHA `{source['coverage']['pair_union_sha256']}`; scores SHA `{source['coverage']['scores_sha256']}`.\n\nInterpretation: Imported CE score artifacts were not modified and CE inference was not rerun.",
        )
    )
    sec.append(
        (
    ...
    sec.append(
        (
            "5 Pointwise Results",
            f"Observed result: {_metric(mapped['pointwise'])} Pointwise vs LambdaMART dNDCG@10 = {pw_delta:.6f}.\n\nInterpretation: Pointwise is competitive but does not beat LambdaMART on aggregate NDCG@10.",
        )
    )
    sec.append(
        (
            "6 LambdaMART Results",
            f"Observed result: {_metric(mapped['lambdamart'])} LambdaMART vs Weighted Hybrid dNDCG@10 = {lm_delta:.6f}.\n\nInterpretation: LambdaMART is the best aggregate M3 method by test NDCG@10.",
        )
    )
    sec.append(
        (
            "7 CE-A Results",
            f"Observed result: {_metric(mapped['hybrid_to_cross_encoder'])} Hybrid->CE vs Weighted Hybrid dNDCG@10 = {ce_a_delta:.6f}. CE-A reranks Hybrid top 100.\n\nInterpretation: CE-A regresses in this cascade, consistent with objective/domain mismatch.",
        )
    )
    sec.append(
        (
            "8 CE-B Results",
            f"Observed result: {_metric(mapped['hybrid_to_lambdamart_to_cross_encoder'])} Hybrid->LambdaMART->CE vs LambdaMART dNDCG@10 = {ce_b_delta:.6f}. CE-B reranks LambdaMART top 50.\n\nInterpretation: CE-B is the largest aggregate regression.",
        )
    )
    sec.append(
        (
    ...
    sec.append(
        (
            ...
            f"Observed result: low lexical overlap H->CE dNDCG@10 = -0.140717 and H->L->CE dNDCG@10 = -0.195133; high disagreement H->CE dNDCG@10 = -0.138625 and H->L->CE dNDCG@10 = -0.224976. Navigational slice is NOT_IMPLEMENTED.\n\n{_slice_notes(slices)}\n\nInterpretation: These patterns are consistent with objective/domain mismatch, without causal overclaiming.",
        )
    )
    sec.append(
        (
    ...
    sec.append(
        (
            ...
            f"Observed result: see `{paths['analysis'] / 'representative_examples.md'}` sections `ce_a_largest_wins_vs_hybrid` and `ce_b_largest_wins_vs_lambdamart`.\n\nInterpretation: These are illustrative examples, not aggregate proof.",
        )
    )
    sec.append(
        (
            "15 Representative Failures",
            f"Observed result: see `{paths['analysis'] / 'representative_examples.md'}` sections `ce_a_largest_losses_vs_hybrid`, `ce_b_largest_losses_vs_lambdamart`, `high_scoring_I`, `low_scoring_E`, `S_C_inversion`, and `keyword_compatibility_or_product_type_mismatch`.\n\nInterpretation: Failures are consistent with objective/domain mismatch and product-type confusion, not causal proof.",
        )
    )
    sec.append(
        (
            "16 Negative Results",
            f"Observed result: both CE cascades reduce test NDCG@10 versus immediate predecessors. CE-A {ce_a_delta:.6f}; CE-B {ce_b_delta:.6f}. CE-score feature ablation is `{ablate['status']}`.\n\nInterpretation: The pretrained CE is not a drop-in M3 improvement.",
        )
    )
    sec.append(
        (
    ...
    sec.append(
        (
            ...
            f"Observed result: final table `{paths['analysis'] / 'final_comparison_table.md'}`; audit `{paths['analysis'] / 'ce_correctness_audit.json'}`; score by label `{paths['analysis'] / 'score_distribution_by_label.md'}`; displacement `{paths['analysis'] / 'rank_displacement_summary.md'}`; slices `{paths['analysis'] / 'query_slice_summary.md'}`; quality latency `{paths['analysis'] / 'quality_latency_summary.md'}`; representative examples `{paths['analysis'] / 'representative_examples.md'}`; source JSON `{paths['analysis'] / 'm3_findings_source.json'}`.\n\nFull final comparison:\n\n{_final_table(rows)}\n\nInterpretation: These generated artifacts support the values in this report.",
        )
    )
